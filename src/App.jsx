...
const PARKS = {
  mk: { name:"Magic Kingdom", emoji:"🏰", color:P.pink,
    rides:[
      ...
      {name:"Jungle Cruise",tier:"Tier 1",ll:"LLMP",avgWait:40,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Classic skipper-guided cruise. Book LLMP early, wait climbs fast midday."},
      {name:"Barnstormer",tier:"Tier 2",ll:"LLMP",avgWait:20,llPrice:"in LLMP",height:'35"',thrill:"Low",tips:"Kid-friendly starter coaster. Short line most of the day."},
      {name:"Buzz Lightyear's Space Ranger Spin",tier:"Tier 2",ll:"LLMP",avgWait:25,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Interactive shooter ride, fun for all ages and rarely a long standby."},
      {name:"Dumbo the Flying Elephant",tier:"Tier 2",ll:"LLMP",avgWait:20,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Has an indoor play area queue, good with toddlers."},
      {name:"It's a Small World",tier:"Tier 2",ll:"LLMP",avgWait:20,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Reliable low-wait option any time of day."},
      {name:"Mad Tea Party",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Classic spinner, skip if prone to motion sickness."},
      {name:"Carpets of Aladdin",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Easy filler ride in Adventureland, rarely a wait."},
      {name:"Winnie the Pooh",tier:"Tier 2",ll:"LLMP",avgWait:25,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Gentle dark ride, popular with young kids."},
      {name:"Mickey's PhilharMagic",tier:"Tier 2",ll:"LLMP",avgWait:20,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"3D show in an air-conditioned theater, great midday break."},
      {name:"Monsters Inc. Laugh Floor",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Interactive comedy show and a good AC break."},
      {name:"Tomorrowland Speedway",tier:"Tier 2",ll:"LLMP",avgWait:20,llPrice:"in LLMP",height:'32"',thrill:"Low",tips:"Kids love driving, line moves slowly but steadily."},
      {name:"Under the Sea: Journey of The Little Mermaid",tier:"Tier 2",ll:"LLMP",avgWait:30,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Charming dark ride in Fantasyland, moderate waits midday."},
    ]},
  ep: { name:"EPCOT", emoji:"🌍", color:P.lilac,
    rides:[
      ...
      {name:"Disney & Pixar Short Film Festival",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Short 3D film program, easy break and rarely crowded."},
      {name:"Journey Into Imagination with Figment",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Low-key dark ride, good filler between headliners."},
      {name:"Living with the Land",tier:"Tier 2",ll:"LLMP",avgWait:20,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Relaxing boat ride through the greenhouses, good midday break."},
      {name:"The Seas with Nemo & Friends",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Gentle ride into the aquarium, pairs well with Turtle Talk."},
      {name:"Spaceship Earth",tier:"Tier 2",ll:"LLMP",avgWait:25,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Iconic geosphere ride, best early or late to avoid the biggest crowds."},
      {name:"Turtle Talk with Crush",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Interactive character show, check showtimes as it fills up fast."},
    ]},
  hs: { name:"Hollywood Studios", emoji:"🎬", color:P.peach,
    rides:[
      ...
      {name:"Alien Swirling Saucers",tier:"Tier 2",ll:"LLMP",avgWait:25,llPrice:"in LLMP",height:'32"',thrill:"Low",tips:"Family spinner in Toy Story Land, long midday waits."},
      {name:"Beauty and the Beast Live on Stage",tier:"Tier 2",ll:"LLMP",avgWait:10,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Live musical, arrive 20 to 30 minutes early for good seats."},
      {name:"Disney Jr. Mickey Mouse Clubhouse Live!",tier:"Tier 2",ll:"LLMP",avgWait:10,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Great for toddlers, check showtimes."},
      {name:"For the First Time in Forever: A Frozen Sing-Along",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Indoor sing-along show and a good AC break."},
      {name:"Indiana Jones Epic Stunt Spectacular",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Live stunt show, arrive early for shade and seating."},
      {name:"The Little Mermaid: A Musical Adventure",tier:"Tier 2",ll:"LLMP",avgWait:20,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Indoor theater show, a comfortable break from the heat."},
      {name:"Star Tours: The Adventures Continue",tier:"Tier 2",ll:"LLMP",avgWait:25,llPrice:"in LLMP",height:'40"',thrill:"Moderate",tips:"Randomized simulator ride, re-rideable with different scenes."},
      {name:"Toy Story Mania!",tier:"Tier 2",ll:"LLMP",avgWait:45,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Hugely popular, book LLMP as early as possible."},
    ]},
  ak: { name:"Animal Kingdom", emoji:"🦁", color:P.mint,
    rides:[
      ...
      {name:"Feathered Friends in Flight",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Live bird show, arrive early as shaded seating fills fast."},
      {name:"Festival of the Lion King",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"High-energy theater show, arrive 20+ minutes early."},
      {name:"Finding Nemo: The Big Blue...and Beyond",tier:"Tier 2",ll:"LLMP",avgWait:15,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Puppet musical in an air-conditioned theater, great midday break."},
      {name:"Kali River Rapids",tier:"Tier 2",ll:"LLMP",avgWait:30,llPrice:"in LLMP",height:'38"',thrill:"Moderate",tips:"You will get soaked, bring a poncho or dry bag."},
      {name:"Zootopia: Better Zoogether!",tier:"Tier 2",ll:"LLMP",avgWait:20,llPrice:"in LLMP",height:"none",thrill:"Low",tips:"Newer stage show, check current showtimes in the app."},
    ]},
};
...
function LightningLaneTab() {
  ...
  const selectRide = (id, rideName) => {
    const slot = dayPlan.find(s=>s.id===id);
    const parkRides = PARKS[slot?.park]?.rides || [];
    const r = parkRides.find(x=>x.name===rideName);
    const newType = r ? (r.ll==="LLSP" ? "LLSP" : "LLMP") : "LLMP";
    setPlans({...plans, [selectedDay]: dayPlan.map(s=>s.id===id?{...s, ride:rideName, type:newType}:s)});
  };
  const removeSlot = (id) => {
  ...
  // Multipass Tier 1 / Tier 2 limits, grouped by park
  const tierCounts = {};
  dayPlan.forEach(s=>{
    if (s.type !== "LLMP" || !s.ride) return;
    const r = (PARKS[s.park]?.rides || []).find(x=>x.name===s.ride);
    if (!r) return;
    if (!tierCounts[s.park]) tierCounts[s.park] = {tier1:0, tier2:0};
    if (r.tier === "Tier 1") tierCounts[s.park].tier1++;
    else if (r.tier === "Tier 2") tierCounts[s.park].tier2++;
  });
  const tierWarnings = Object.entries(tierCounts).filter(([pk,c])=>{
    const maxTier2 = c.tier1 > 0 ? 2 : 3;
    return c.tier1 > 1 || c.tier2 > maxTier2;
  }).map(([pk])=>PARKS[pk]?.name);
  ...
  return (
    <div>
      ...
      {Object.keys(tierCounts).length > 0 && (
        <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:10,fontSize:11,color:P.mid,fontWeight:700}}>
          {Object.entries(tierCounts).map(([pk,c])=>(
            <span key={pk}>{PARKS[pk]?.emoji} {PARKS[pk]?.name}: Tier 1 {c.tier1}/1 · Tier 2 {c.tier2}/{c.tier1>0?2:3}</span>
          ))}
        </div>
      )}
      {tierWarnings.length > 0 && (
        <div style={{background:"#FDEDEC",border:"1.5px solid #E74C3C",borderRadius:10,padding:"10px 14px",marginBottom:12,fontSize:12,fontWeight:700,color:"#C0392B"}}>
          ⚠️ Multipass limit exceeded for {tierWarnings.join(", ")} — Multipass allows 1 Tier 1 pick + 2 Tier 2 picks (or 3 from Tier 2) per park per day.
        </div>
      )}
          ...
          return (
            <Card key={slot.id} style={{borderLeft:`5px solid ${hasConflict?"#E74C3C":parkColor}`,padding:"12px 16px"}}>
              <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                ...
                <span style={{padding:"7px 10px",borderRadius:8,border:`1.5px solid ${P.border}`,fontSize:12,fontWeight:700,color:"#fff",background:slot.type==="LLSP"?P.dark:P.mid,flexShrink:0}}>
                {slot.type==="LLSP" ? "⚡⚡ Single Pass" : "⚡ Multipass"}
              </span>
              ...
              <select value={slot.ride} onChange={e=>selectRide(slot.id,e.target.value)}
                ...
                {PARKS[slot.park]?.rides.filter(r=>r.ll!=="None").map(r=><option key={r.name} value={r.name}>{r.name} — {r.ll==="LLSP"?"Single Pass":r.tier}</option>)}
              </select>
              {hasConflict && (
...
const RESORT_TRANSPORT_ROUTES = [
  { name:"Contemporary Resort", tier:"Deluxe", routes:[
    {park:"mk", method:"Monorail", icon:"🚝", time:"5\u201310 min (or walk!)"},
    {park:"ep", method:"Monorail", icon:"🚝", time:"20\u201325 min (transfer at TTC)"},
  ]},
  { name:"Bay Lake Tower", tier:"Deluxe Villas", routes:[
    {park:"mk", method:"Monorail", icon:"🚝", time:"5\u201310 min (or walk!)"},
    {park:"ep", method:"Monorail", icon:"🚝", time:"20\u201325 min (transfer at TTC)"},
  ]},
  { name:"Polynesian Village Resort", tier:"Deluxe", routes:[
    {park:"mk", method:"Monorail", icon:"🚝", time:"10 min"},
    {park:"mk", method:"Boat", icon:"\u26f4️", time:"15 min"},
    {park:"ep", method:"Monorail", icon:"🚝", time:"20\u201325 min (transfer at TTC)"},
  ]},
  { name:"Grand Floridian Resort & Spa", tier:"Deluxe", routes:[
    {park:"mk", method:"Monorail", icon:"🚝", time:"10 min"},
    {park:"mk", method:"Boat", icon:"\u26f4️", time:"15 min"},
    {park:"ep", method:"Monorail", icon:"🚝", time:"20\u201325 min (transfer at TTC)"},
  ]},
  { name:"Wilderness Lodge", tier:"Deluxe", routes:[
    {park:"mk", method:"Boat", icon:"\u26f4️", time:"15\u201320 min"},
  ]},
  { name:"Fort Wilderness Resort & Campground", tier:"Value / Cabins", routes:[
    {park:"mk", method:"Boat", icon:"\u26f4️", time:"20\u201325 min"},
  ]},
  { name:"Caribbean Beach Resort", tier:"Moderate", routes:[
    {park:"ep", method:"Skyliner", icon:"🚡", time:"15 min"},
    {park:"hs", method:"Skyliner", icon:"🚡", time:"15\u201320 min"},
  ]},
  { name:"Riviera Resort", tier:"Deluxe Villas", routes:[
    {park:"ep", method:"Skyliner", icon:"🚡", time:"10 min (direct)"},
    {park:"hs", method:"Skyliner", icon:"🚡", time:"15 min (transfer at Caribbean Beach)"},
  ]},
  { name:"Pop Century Resort", tier:"Value", routes:[
    {park:"ep", method:"Skyliner", icon:"🚡", time:"20\u201325 min (transfer at Caribbean Beach)"},
    {park:"hs", method:"Skyliner", icon:"🚡", time:"20\u201325 min (transfer at Caribbean Beach)"},
  ]},
  { name:"Art of Animation Resort", tier:"Value", routes:[
    {park:"ep", method:"Skyliner", icon:"🚡", time:"20\u201325 min (transfer at Caribbean Beach)"},
    {park:"hs", method:"Skyliner", icon:"🚡", time:"20\u201325 min (transfer at Caribbean Beach)"},
  ]},
  { name:"Yacht Club Resort", tier:"Deluxe", routes:[
    {park:"ep", method:"Boat", icon:"\u26f4️", time:"10 min (or walk!)"},
    {park:"hs", method:"Boat", icon:"\u26f4️", time:"15\u201320 min"},
  ]},
  { name:"Beach Club Resort", tier:"Deluxe", routes:[
    {park:"ep", method:"Boat", icon:"\u26f4️", time:"10 min (or walk!)"},
    {park:"hs", method:"Boat", icon:"\u26f4️", time:"15\u201320 min"},
  ]},
  { name:"BoardWalk Inn & Villas", tier:"Deluxe", routes:[
    {park:"ep", method:"Boat", icon:"\u26f4️", time:"10 min (or walk!)"},
    {park:"hs", method:"Boat", icon:"\u26f4️", time:"15\u201320 min (or walk!)"},
  ]},
  { name:"Walt Disney World Swan", tier:"Deluxe (Partner)", routes:[
    {park:"ep", method:"Boat", icon:"\u26f4️", time:"10\u201315 min"},
    {park:"hs", method:"Boat", icon:"\u26f4️", time:"15\u201320 min"},
  ]},
  { name:"Walt Disney World Dolphin", tier:"Deluxe (Partner)", routes:[
    {park:"ep", method:"Boat", icon:"\u26f4️", time:"10\u201315 min"},
    {park:"hs", method:"Boat", icon:"\u26f4️", time:"15\u201320 min"},
  ]},
];
...
function TransportationTab() {
  ...
  const [resort, setResort] = useState(RESORT_TRANSPORT_ROUTES[0].name);
  ...
  return (
    <div>
      ...
      {view==="resort" && (
        <div>
          <Callout icon="🏨">
            These resorts connect directly to the parks via <strong>Monorail</strong>, <strong>Boat</strong>, or <strong>Skyliner</strong> — no bus required. Pick a resort to see its routes, or pick a park to see which resorts serve it.
          </Callout>

          <label style={{fontSize:11,fontWeight:700,color:P.gray,textTransform:"uppercase",letterSpacing:.6,display:"block",marginBottom:6,marginTop:14}}>Resort</label>
          <select value={resort} onChange={e=>setResort(e.target.value)}
            style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`2px solid ${P.pink}`,fontSize:14,fontWeight:700,color:P.dark,background:P.cream,marginBottom:16}}>
            {RESORT_TRANSPORT_ROUTES.map(r=><option key={r.name} value={r.name}>{r.name}</option>)}
          </select>

          {RESORT_TRANSPORT_ROUTES.find(r=>r.name===resort)?.routes.map((rt,i)=>(
            <Card key={i} style={{marginBottom:10,display:"flex",alignItems:"center",gap:12}}>
              <div style={{fontSize:24}}>{rt.icon}</div>
              <div style={{flex:1}}>
                <div style={{fontWeight:800,fontSize:14,color:P.dark}}>{PARKS[rt.park]?.emoji} {PARKS[rt.park]?.name}</div>
                <div style={{fontSize:12,color:P.mid}}>{rt.method} · {rt.time}</div>
              </div>
            </Card>
          ))}

          <div style={{marginTop:24}}>
            <label style={{fontSize:11,fontWeight:700,color:P.gray,textTransform:"uppercase",letterSpacing:.6,display:"block",marginBottom:6}}>Which resorts reach {PARKS[to]?.name}?</label>
            <select value={to} onChange={e=>setTo(e.target.value)}
              style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`2px solid ${P.pink}`,fontSize:14,fontWeight:700,color:P.dark,background:P.cream,marginBottom:12}}>
              {parks.map(p=><option key={p} value={p}>{PARKS[p]?.emoji} {PARKS[p]?.name}</option>)}
            </select>
            {RESORT_TRANSPORT_ROUTES.filter(r=>r.routes.some(rt=>rt.park===to)).map(r=>(
              <div key={r.name} style={{display:"flex",justifyContent:"space-between",padding:"8px 12px",borderBottom:`1px solid ${P.border}`,fontSize:13}}>
                <span style={{fontWeight:700,color:P.dark}}>{r.name}</span>
                <span style={{color:P.mid}}>{r.routes.filter(rt=>rt.park===to).map(rt=>`${rt.method} (${rt.icon})`).join(" · ")}</span>
              </div>
            ))}
            {RESORT_TRANSPORT_ROUTES.filter(r=>r.routes.some(rt=>rt.park===to)).length===0 && (
              <div style={{textAlign:"center",padding:20,color:P.gray,fontSize:13}}>No direct Monorail, Boat, or Skyliner route to this park — bus is the way to go from most resorts.</div>
            )}
          </div>
        </div>
      )}
